...
The ``uuid`` on iOS uses the identifierForVendor property. It is unique
to the device across the same vendor, but will be different for
different vendors and will change if all apps from the vendor are
...
(`SERIAL <http://developer.android.com/reference/android/os/Build.html#SERIAL>`__).
